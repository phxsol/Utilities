...
console.log("pattern", "proliferation", "instances");
var obj = JSON.parse(fs.readFileSync('tokens_patterns_phrases.json', 'utf8'));
var pattern_relevance = obj.patterns.map(mapRelevance);
function mapRelevance(element, index, array){
    var a = [];
    a.push(element.pattern);
    a.push(element.use_cases.length);
    a.push(element.use_cases.reduce(totalUses, 0));
    console.log("\"" + a.join("\",\"") + "\"");
    return a;
}
var a = 1;
function totalUses(total, curr_value){
    return total + curr_value.uses;
}

function spaz_tokens_inator() {
